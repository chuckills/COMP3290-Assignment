...
public class OutputController
{
	...
	private int line;		// current line number of source file -> listing
	private int charPos;		// character position within listing line
	private int errorCount;		// count of errors for entire compiler
	private PrintWriter listing;		// descriptor of the listing file
	private StringBuffer err;		// reference to error StringBuffer
	private String currLine;		// current listing line being built
	private String errLine;		// errors associated with current listing line
	...
	public void printChar(char ch) // stores next char - Prints a listing line if a newline char
    {
		...
		if (ch == '\n') // at newline - produce the next line of the listing
		{
			...
			line++;
			if (line < 10) // Initialise buffer to new line number
            {
                currLine = "  " + line + ": ";
            }
			else if (line < 100)
